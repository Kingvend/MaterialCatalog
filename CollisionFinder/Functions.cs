...
namespace CollisionFinder
{
    static class Functions
    {
        ...
        static List<DB.CustomHistory> CatalogForYearDB(List<DB.CustomHistory> catalogs, int year)
        {
            var catalog = new List<DB.CustomHistory>();
            int year1;
            ...
            foreach (var c in catalogs)
            {
                //month1 = Int32.Parse(c.DeliveryDate.Substring(3, 2));
                //year1 = Int32.Parse(c.DeliveryDate.Substring(6, 4));
                year1 = Int32.Parse(c.DATE_SCHF.Substring(6, 4));
                if (year1 == year)
                {
                    catalog.Add(c);
                }
        ...
        public static double SumMtrDB(List<DB.CustomHistory> catalogs, double Koef1, double Koef2, int CurentYear)
        {
            var Catalog1 = new List<DB.CustomHistory>();
            var Catalog2 = new List<DB.CustomHistory>();
            var Catalog3 = new List<DB.CustomHistory>();
            NumberFormatInfo provider = new NumberFormatInfo();
            provider.NumberDecimalSeparator = ",";
            ...
            // code
            // 3 catalogs for last 3 year
            Catalog1.AddRange(CatalogForYearDB(catalogs, CurentYear));
            Catalog2.AddRange(CatalogForYearDB(catalogs, CurentYear - 1));
            Catalog3.AddRange(CatalogForYearDB(catalogs, CurentYear - 2));

            foreach (var s in Catalog1)
            {
                double tmpSum, tmpV;
                tmpSum = Convert.ToDouble(s.SUM_SCHF, provider);
                Sum1 += tmpSum;
                tmpV = Convert.ToDouble(s.KOL_SCHF, provider);
                V1 += tmpV;
            }
